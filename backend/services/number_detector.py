...
def detect_all(conf: float = 0.25, imgsz: int = 640, results_dir: Optional[Path] = None, input_path: Optional[Path] = None):
    """
    Use static/models/number_best.pt to detect images located in static/results/crops (by default).
    - conf: confidence threshold passed to model.predict (can be lower than MIN_KEEP_CONF)
    - imgsz: inference image size
    - results_dir: optional output directory (defaults to static/results/number_detect)
    - input_path: optional single image to process (bypass crops folder)
    """
    ...
    if input_path:
        ...
        if not crops_dir.exists():
            print("Crops directory not found:", crops_dir)
            return
